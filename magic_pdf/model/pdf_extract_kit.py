...
class CustomPEKModel:
    ...
    def __call__(self, images):
        ...
        layout_res = self.layout_model(images, ignore_catids=[])
        layout_cost = round(time.time() - layout_start, 2)
        logger.info(f"layout detection cost: {layout_cost}")

        if self.apply_formula:
            # 公式检测
            mfd_results = self.mfd_model.predict(images, imgsz=1888, conf=0.25, iou=0.45, verbose=True)
            assert len(images) == len(mfd_results)
            for image_index, mfd_res in enumerate(mfd_results):
                for xyxy, conf, cla in zip(mfd_res.boxes.xyxy.cpu(), mfd_res.boxes.conf.cpu(), mfd_res.boxes.cls.cpu()):
                    ...
                    bbox_img = get_croped_image(Image.fromarray(images[image_index]), [xmin, ymin, xmax, ymax])
