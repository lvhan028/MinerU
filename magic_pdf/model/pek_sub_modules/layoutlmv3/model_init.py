...
class Layoutlmv3_Predictor(object):
    ...
    def __call__(self, images, ignore_catids=[]):
        # page_layout_result = {
        #     "layout_dets": []
        # }
        layout_dets = []
        ...
        for image_index, _outputs in enumerate(outputs):
            boxes = _outputs["instances"].to("cpu")._fields["pred_boxes"].tensor.tolist()
            labels = _outputs["instances"].to("cpu")._fields["pred_classes"].tolist()
            scores = _outputs["instances"].to("cpu")._fields["scores"].tolist()
